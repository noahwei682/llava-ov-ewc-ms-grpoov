...
class LLaVATrainerWithEWC(LLaVATrainer):
    def __init__(self, *args, **kwargs):
        ...
        self.use_ewc = getattr(self.args, 'use_ewc', False)
        self.ewc_lambda = getattr(self.args, 'ewc_lambda', 0.1)
        self.fisher_dict = {}
        self.optpar_dict = {}
        self.max_layers = 12
        self.batch_size = 4
    ...
    def _initialize_target_layers(self, model):
        """Dynamically detect target layer patterns"""
        layer_patterns = defaultdict(int)
        for name in model.state_dict():
            if re.search(r'layers\.\d+\.', name):
                generic_pattern = re.sub(r'\.\d+\.', r'\.\\d+\.', name)
                layer_patterns[generic_pattern] += 1
                
        # Select top 8 most frequent patterns
        self.target_layers = sorted(layer_patterns, 
                                  key=layer_patterns.get, 
                                  reverse=True)[:8]
        print(f"🔍 Detected target layer patterns: {self.target_layers}")

    def _is_visual_param(self, name):
        """Advanced visual parameter detection"""
        visual_patterns = [
            r'vision_tower', 
            r'visual_encoder',
            r'image_proj',
        ...
        return any(re.search(p, name) for p in visual_patterns)
    ...
    def _select_parameters(self, model):
        ...
        for name, param in model.named_parameters():
            if not param.requires_grad:
                exclusion_reasons['requires_grad=False'] += 1
                continue
                
            if self._is_visual_param(name):
                exclusion_reasons['visual_param'] += 1
                continue
                
            if any(re.search(p, name) for p in self.target_layers):
                if param.numel() > 0:
                    selected[name] = param
                else:
                    exclusion_reasons['zero_size'] += 1
            else:
                exclusion_reasons['pattern_mismatch'] += 1

        # Stage 2: Layer-limited selection
        if not selected:
            print("⚠️ Trying layer-limited selection...")
            for name, param in model.named_parameters():
                if param.requires_grad and not self._is_visual_param(name):
                    if re.search(r'layers\.\d+\.', name):
                        selected[name] = param

        # Stage 3: Fallback to all non-visual
        if not selected:
            print("⚠️ Fallback to all non-visual parameters")
            selected = {n: p for n, p in model.named_parameters()
                       if p.requires_grad and not self._is_visual_param(n)}
    ...
    def compute_fisher(self, model):
        """Robust Fisher matrix calculation"""
        try:
            self._validate_parameters(model)
            self._initialize_target_layers(model)
            trainable_params = self._select_parameters(model)
            ...
            fisher_dict = {n: torch.zeros_like(p.data, device='cpu')
                          for n, p in trainable_params.items()}
            optpar_dict = {n: p.data.cpu().clone()
                          for n, p in trainable_params.items()}
            ...
            for batch_idx, batch in enumerate(islice(dataloader, self.num_batches)):
                ...
                for name, param in trainable_params.items():
                    if param.grad is not None:
                        fisher_dict[name] += param.grad.cpu().pow(2)
                        
                print(f"📦 Processed batch {batch_idx+1}/{self.num_batches}")

            return fisher_dict, optpar_dict

        except Exception as e:
            # print(f"❌ Fisher computation failed: {str(e)}")
            return {}, {}

    def train(self, *args, **kwargs):
        """Enhanced training entry point"""
        if self.use_ewc:
            try:
                print("\n🔧 Initializing EWC...")
...
class ParameterMonitor:
    """Gradient flow monitoring toolkit"""
    def __init__(self):
        self.hooks = []
        ...
        for name, param in model.named_parameters():
            if param.requires_grad:
                hook = param.register_hook(
                    lambda grad, name=name: self._analyze_grad(grad, name))
                self.hooks.append(hook)
                
    def _analyze_grad(self, grad, name):
        ...
        # Log statistics
        self._log_grad_stats(name, grad_norm, grad_mean)

    def _log_grad_stats(self, name, norm, mean):
